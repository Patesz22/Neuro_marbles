		...
		if (hasCommand)
		{
			if (currentCmd.CommandType == "STATE")
			{
				try
				{
					...
					state.CurrentState = static_cast<ENeuroState>(newState);
			...
			else if (currentCmd.CommandType == "BUTTON")
			{
				...
				PressInGameButton(currentCmd.Payload);
			...
			else if (currentCmd.CommandType == "ACTION")
			{
				...
				// might need to inject fake JSON into state.lastData if it requires parameters.
				state.LastNeuroAction = currentCmd.Payload;
